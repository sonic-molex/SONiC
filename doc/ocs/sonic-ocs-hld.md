...
- [SONiC-OCS HLD](#sonic-ocs-hld)
    ...
    - [7 High-Level/Module Design](#7-high-levelmodule-design)
      ...
      - [7.5 PMON](#75-pmon)
        ...
        - [7.5.4 Firmare Upgrade](#754-firmare-upgrade)
...
For OCS device, Syncd behaviors exactly same. But instead of managing the ASIC, each vendor will implement SAI OCS extension APIs to control and monitor the OCS HW, MEMS or LCoS mirror array. Notification handlers are also registered to handle events from the HW. OCS can be supported by extending the logic to processing new SAI APIs for OCS objects. See [here](https://github.com/sonic-molex/sonic-sairedis/blob/ocs/syncd/FlexCounterOtai.cpp).
...
An example of driver of PMON is [here](https://github.com/sonic-molex/sonic-buildimage/tree/ocs/platform/ocs-molex/sonic-platform-modules-molex/ols).
...
<img src="./images/line-card-daemon.png" alt="Redis script" width="600">

As shown in the above diagram, a new line card monitoring daemon is added to PMON cotainer for line card operation status monitoring.

Line card un-plugg/failed:
- PMON detecte a line card is removed. It change status of a line card from online to `empty/fault`. The line card daemon should update all the ports that is affected by this line card by changing the port admin state (port-config-override-state) in APP DB to `powered-off (offline??)`. 
- Orchagent (oscportorch) will be triggered to remove responding SAI port objects and associated flexcouter ebtries.
...
- If the port admin state is `normal`, Orchagent (oscportorch) will be triggered to create responding SAI port objects and associated flexcouter ebtries.
...
##### 7.5.4 Firmare Upgrade

SONiC provide a generic mechanism to install/upgrade firmware, [fwutil](https://github.com/sonic-net/SONiC/blob/master/doc/fwutil/fwutil.md).

OCS vendor need to implement the python component APIs defined in the base class [`componets_base.py`](https://github.com/sonic-net/sonic-platform-common/blob/master/sonic_platform_base/component_base.py):
...
Most sonic CLI is implemented in sonic-utility based on [python click library](https://click.palletsprojects.com/en/8.1.x/). These CLI are supported in [sonic-utilities.](https://github.com/sonic-net/sonic-utilities). It is preferred that OCS CLI following this implementation.
...
The following OCS commands will be added after `sonic-app-cli` is installed:
...
##### 9.2.3 REST API
ONiC management framework infrastructure's Translib converts the data models exposed to the management clients into the Redis ABNF schema format. See HLD [here](https://github.com/sonic-net/SONiC/blob/master/doc/mgmt/Management%20Framework.md).
