...
- [1 Requirements Overview](#1-requirements-overview)
  ...
  - [1.2 Warm Restart requirements](#12-warm-restart-requirements)
  - [1.3 Scaling requirements](#13-scaling-requirements)
  - [1.4 Alarm](#14-alarm)
  - [1.5 PM Counter](#15-pm-counter)
  - [1.6 Telemetry](#16-telemetry)
- [2 SONiC Extension For OCS](#2-sonic-extension-for-ocs)
  - [2.1 Design Principles](#21-design-principles)
  - [2.2 SONiC Extension Points for OCS Support](#22-sonic-extension-points-for-ocs-support)
- [3 Modules Design](#3-modules-design)
  - [3.1 Northbound Management Interface](#31-northbound-management-interface)
    - [3.1.1 OCS SONiC Yang Model](#311-ocs-sonic-yang-model)
    - [3.1.2 REST API](#312-rest-api)
    - [3.1.3 CLI](#313-cli)
  - [3.3 Redis DB Schema](#33-redis-db-schema)
    - [3.3.1 OCS Device Metadata](#331-ocs-device-metadata)
    - [3.3.2 Config And State DB](#332-config-and-state-db)
  - [3.4 SAI Extension for OCS](#34-sai-extension-for-ocs)
    - [3.4.1 SAI Experimental Extension Mechanism](#341-sai-experimental-extension-mechanism)
    - [3.4.2 OCS Extension To SAI](#342-ocs-extension-to-sai)
  - [3.5 SWSS Extension for OCS Optical Traffic](#35-swss-extension-for-ocs-optical-traffic)
    - [3.5.1 SWSS Config Manager](#351-swss-config-manager)
    - [3.5.2 SWSS OrchAgent](#352-swss-orchagent)
      - [OrchDaemon for OCS](#orchdaemon-for-ocs)
    - [Generic Orchagent Class](#generic-orchagent-class)
  - [3.6 OCS State and PM Statistics Support](#36-ocs-state-and-pm-statistics-support)
    - [SONiC SWSS Redis plug-in script](#sonic-swss-redis-plug-in-script)
    - [Scripts update the State DB Example](#scripts-update-the-state-db-example)
  - [3.7 SyncD Extension for OCS](#37-syncd-extension-for-ocs)
  - [3.8 PMON Enhancement](#38-pmon-enhancement)
  - [3.9 SONiC host containers](#39-sonic-host-containers)
...
Another OCS use case to provide low power consumption and low latency inter data center connections, shown in the following diagram:[(source):](https://www.microsoft.com/en-us/research/publication/beyond-the-mega-data-center-networking-multi-data-center-regions/)
...
## 1.2 Warm Restart requirements
Warm-restart should be a non traffic effect operation.
...
* All changes should be compatible to the upstream SONiC code base and ready to be merged. The final is that supporting OCS be part of SONiC NOS.
	
## 2.2 SONiC Extension Points for OCS Support
...
## 3.1 Northbound Management Interface
### 3.1.1 OCS SONiC Yang Model
...
- CLI auto generation:
    ...
    - Auto generate CLI based on Openconfig yang and its annotation.
...
### 3.3.2 Config And State DB
Config DB and state DB schema are strictly mapped from sonic-ocs.yang:
```
CONFIG_DB
=========
OCS_PORT
...
## 3.4 SAI Extension for OCS

### 3.4.1 SAI Experimental Extension Mechanism
...
## 3.5 SWSS Extension for OCS Optical Traffic
...
### 3.5.1 SWSS Config Manager

In SWSS container, a new config manager daemon, [`otaimgrd`](https://github.com/sonic-molex/sonic-swss/blob/ocs/cfgmgr/otaimgrd.cpp), is created to subscribe the changes in OCS tables in config DB. When config change is notified, OCS config manager update the corresponding tables in APP DB. Nota that otai (optical transport abstraction API) is used for covering other optical device, OLS and transponder. 

### 3.5.2 SWSS OrchAgent 
...
Instead of hard code the all SAI object attributes in a static map table, archagent corresponding to each SAI object can re-use otaiobjectorch for flexiblecounter DB access.

## 3.6 OCS State and PM Statistics Support
...
As SONiC currently does not support PM. (OTN syncd added historical data by modifying the syncd code base. This causes a code base diverge from the upstream code).

### SONiC SWSS Redis plug-in script
...
Examples of Lua scripts within the SWSS can be found in the sonic-swss repository. One notable example is pfc_restore.lua, which uses Redis commands to handle PFC (Priority Flow Control) restoration.
...
### Scripts update the State DB Example
...
## 3.7 SyncD Extension for OCS

## 3.8 PMON Enhancement

## 3.9 SONiC host containers
