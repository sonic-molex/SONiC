...
(Comply to [SONiC HLD Template](https://github.com/sonic-net/SONiC/blob/master/doc/hld_template.md))
...
- [SONiC-OCS HLD](#sonic-ocs-hld)
  ...
  - [7 High-Level/Module Design](#7-high-levelmodule-design)
    ...
    - [7.5 PMON Enhancement](#75-pmon-enhancement)
      - [7.5.1 PMON Base Class](#751-pmon-base-class)
      - [7.5.2  Device specific platform config](#752--device-specific-platform-config)
      - [7.5.3 Linecard Hot-pluggable](#753-linecard-hot-pluggable)
      - [7.5.4 Headless Operation](#754-headless-operation)
...
| :---: | :--------: | :-----------------: | :---------------------------------------- |
|  0.1  | 06/30/2025 |    Jimmy Jin, Lu Mao   | Initial version, some material is derived from [sonic-otn-wp](https://github.com/sonic-otn/SONiC-OTN)|
|       |        |         |        |

## 2 Scope

This document describes the architecture and high level design for extending SONiC to support Optical Circuit Swicth (OCS) device.
...
As such, OCS can be used a various network use cases. One of the main application of OCS is to connect large number of AI computing nodes to form a AI super computing cluster, as shown in the following diagram [(source):](https://arxiv.org/pdf/2304.01433)
  ...
  <img src="./images/ocs-ai.png" alt="OCS for AI" width="400">
...
- Bring up SONiC image for DEVICE_METADATA type - `SonicOt`
...
Following are the minimal scaling requirements: [*TBD*]
...
- Complete reuse SONiC generic system features as is, Including BNI (REST, CLI, gNMI), telemetry, user management, syslog notification, SW/FW upgrade, chassis/PSU/LED/FAN/temperature management, etc.

- Changes for OCS support should be modular and relatively isolated from the packet switching logic with minimum impact on existing packet switching functions.

- For major gaps, such as PM, alarm and hot pluggable, design and implement should be in a generic way, not just for OCS.

- All changes should be compatible to the upstream SONiC code base and ready to be merged. The final goal is that all OCS vendors shoul dbe able to pull the official SONiC code and build SONiC OCS images.
...
In DEVICE meta data table, a new type, `SonicOt`, and new switch_type, `ot`, are added:
...
`ot` stands for optical transport, as the OCS change described in this doc can extended to support other optical devices.

see [change here](https://github.com/sonic-molex/sonic-buildimage/commit/046c3a36dfaa7b0f47536846ffce2c0318d65b12).
...
In SWSS container, a new config manager daemon, [`otaimgrd`](https://github.com/sonic-molex/sonic-swss/blob/ocs/cfgmgr/otaimgrd.cpp), is created to subscribe the changes in OCS tables in config DB. When config change is notified, OCS config manager update the corresponding tables in APP DB. Note that the name OTAI (optical transport abstraction API) is used for covering other optical device, OLS and transponder potentially.

[**TBD**] How the failure is handled, if the config change does not pass the business logic validation? SONiC configuration is managed asychronously, i.e., the config will be accepted and stored in config DB, even the low level/HW processing fails.

#### 7.2.2 SWSS OrchAgent

Orchagent is extended with a [separate folder](https://github.com/sonic-molex/sonic-swss/tree/ocs/orchagent/ot) to support OCS devices.

##### OrchDaemon for OCS

Currently, SONiC support two types of Orch Daemon based on `switchType`, ochaDaemon or fabricOrchDaemon. A new type of orchDaemon, otaiOrchDamon is added to support OCS devices. At run time, switchType == `ot` is use to determine if [oatiOrchDaemon](https://github.com/sonic-molex/sonic-swss/blob/ocs/orchagent/ot/otaiorchdaemon.cpp) should dbe created. please see [code here](https://github.com/sonic-molex/sonic-swss/blob/ocs/orchagent/main.cpp).

``` C++

    if(switchType == "ot")
    {
        create orchDaemon = make_shared<OtaiOrchDaemon>;
    }
...
A generic superclass, [`otaiobjectorch`](https://github.com/sonic-molex/sonic-swss/blob/ocs/orchagent/ot/otaiobjectorch.cpp), is defined to support all operations (CRUD) the Flexcounter DB.

Instead of hard code the all SAI object attributes in a hard coded static map table, archagent corresponding to each SAI object can re-use the generic methods in otaiobjectorch for State and Flexiblecounter DB access. Shown as in the following diagram, where the SAI attributes to string map are built generically by reading the SAI matadata.

<img src="./images/object-orchagent.png" alt="SWSS and SyncD" width="700">

This effectively removed many redundant code and elimineted human error from hard code.
...
SONiC currently does not support PM statis couters. (OTN syncd added historical data by modifying the syncd code base. This causes a code base diverge from the upstream code).

Also, attributes specified in the state container of sonic-ocs.yang need to be updated continously, so that BNI (CLI/REST API) can read the OCS status from State DB directly.
...
It is proposed to use SWSS lua script to support State DB update and PM statistic counters with the following benifits:

- Use existing counter DB as is. SONiC counter DB is designed for storing raw data from the hardware. Syncd updates the counters specified in the Flex-counter DB. Both State and PM counters can be built form the tables in couter DB
...
- These lua scripts can be device specific, i.e., each device vendor can write customized sripts and include as part of the device data. This provides the maximun flexibilty and make status update and PM complete generic.
...
Here is the flex-counter DB and counter DB after script is installed for update the state DB. [example](https://github.com/sonic-molex/sonic-buildimage/tree/ocs/device/molex/x86_64-molex_ocs_16x16-r0)

Note cuuretly. some vendor specific lua scripts are put into in the SWSS [orchagent](https://github.com/sonic-net/sonic-swss/blob/master/orchagent), which is not ideal. They could be included as part of device config, as above example.
...
For OCS device, Syncd behaviors exactly same. But instead of managing the ASIC, each vendor will implement SAI OCS extension APIs to managecontrol and monitor the OCS HW, MEMS or LCoS mirror array. Notification handlers are also registered to handle events from the HW.

Syncd is desinged as a generic control flow for communicating with the Redis database, loading SAI implementation, and interacting with it to handle hardware initialization, configuration, status reporting, and so on. Therefore, OCS can be supported by extending the logic to processing new SAI APIs for OCS objects. See [code here](https://github.com/sonic-molex/sonic-sairedis/blob/ocs/syncd/FlexCounterOtai.cpp).

Since OSC support is added at end of logic, the new OCS code will not have any impact on existing switch functionalities.

<img src="./images/ocs-syncd.png" alt="Redis script" width="600">

### 7.5 PMON Enhancement

SONiC pmon (platform monitor) manages generic hardware, which is independent from the functionality of the device providing. pmon infrastructure is implemented in two repositories, [sonic-platform-common](https://github.com/sonic-net/sonic-platform-common) and [sonic-platform-daemon](https://github.com/sonic-net/sonic-platform-daemons) described in [this doc here](https://github.com/sonic-net/SONiC/blob/master/doc/platform_api/new_platform_api.md). And Vendor platform module resides under ``sonic-buildimage/platform`` folder for each device type.
...
- Chassis
  ...
  - Module[0 .. m-1] (Line card, supervisor card, etc.)
    ...
    - Front-panel/status LEDs
    - SFP cage[0 .. s-1]
    - Components[0 .. n-1] (CPLD, FPGA, MCU, etc.)
      - name
      - description
      - firmware

#### 7.5.2  Device specific platform config

The [JSON file here](https://github.com/sonic-molex/sonic-buildimage/blob/ocs/device/molex/x86_64-molex_ocs_16x16-r0/platform.json) is to define the OCS device HW hierachy described above. This config file is device specific for a particular OCS device.

#### 7.5.3 Linecard Hot-pluggable

Curently, SONiC suppport two chassis types:

- Pizza box without pluggabe supervisor/control card and line cards
- Muti-Asic, in whitch each line cards running an independent SONiC.

In OCS case, SONiC will running on

#### 7.5.4 Headless Operation
...
1. First add new OCS port and connection API types in `saiextension.h`. Note that OTAI is used as part of the prefix to cover all optical devices, such as OLS and Transponders, not just OCS.
...
/* ocs experimental extension */
#include "saiexperimentalotaiocscrossconnect.h"
#include "saiexperimentalotaiocsport.h"
...
typedef enum _sai_api_extensions_t
{
 ...
 ....
    ...
    SAI_API_OTAI_OCS_CROSS_CONNECT,

    SAI_API_OTAI_OCS_PORT,
...
typedef enum _sai_object_type_extensions_t
{
    ...
    SAI_OBJECT_TYPE_OTAI_OCS_CROSS_CONNECT,
    
    SAI_OBJECT_TYPE_OTAI_OCS_PORT,
...
3. The new attributes for OCS SAI API are defined in two new separate head files for modulization.
   ...
   `saiexperimentalotaiocsport.h` ([code](https://github.com/sonic-molex/SAI/blob/ocs/experimental/saiexperimentalotaiocsport.h)) and `saiexperimentalotaiocscrossconnect.h` ([code](https://github.com/sonic-molex/SAI/blob/ocs/experimental/saiexperimentalotaiocscrossconnect.h)).
...
CLI autogeneration app's manifest JSON is [here](https://github.com/sonic-molex/sonic-app-cli/blob/main/manifest.json.j2)
...
Currently, there are two styles of SONiC CLI with different implementations:

1. write CLI (xml, actioner and render) based on [klish framework](https://src.libcode.org/pkun/klish/src/master). This is part of the [sonic-mgmt-framework](https://github.com/sonic-net/SONiC/blob/master/doc/mgmt/Management%20Framework.md#3121-cli).

2. Most sonic CLI is implemented in sonic-utility based on [python click library](https://click.palletsprojects.com/en/8.1.x/). These CLI are supported in [sonic-utilities.](https://github.com/sonic-net/sonic-utilities). It is preferred that OCS CLI following this implementation.

In order to have better mobilization for OCS support. It is proposed that OCS specific CLI should be implemented with the following requirement:

- Pluggable: Instead of mix with the existing SONiC CLI implementation code base, OCS CLI should be a independent module. Moreover it can be a SONiC compatible Docker image, based on [SONiC application extension mechanism](https://github.com/sonic-net/SONiC/tree/master/doc/sonic-application-extension).
...
- CLI auto generation:
  ...
  - Auto generate CLI based on Openconfig yang and its annotation. See openconfig (standard) yang model support in SONiC (<https://github.com/project-arlo/sonic-mgmt-framework/wiki/Transformer-Developer-Guide>).

  <img src="./images/openconfig-cli.png" alt="OCS for DCI" width="600">

  - Provide a way of including other manual written CLIs.

The following OCS commands shall be added :
...
``` CLI
 ...
 - config ocs-port 1B --config-status [force-blocked | normal]
...
Therefore, after sonic-ocs.yang is add into sonic-mgmt-common yang mode [directory](https://github.com/sonic-molex/sonic-mgmt-common/blob/ocs/models/yang/sonic), REST API is supported automatically.
