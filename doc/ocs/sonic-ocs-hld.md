...
- PMON detect a line card is removed. It change status of a line card from online to `empty/fault`. The line card daemon should update all the ports that is affected by this line card by changing the port admin state (port-config-override-state) in APP DB to `powered-off (offline??)`.
